...
private:
  ...
  std::pair<MatrixXd, MatrixXd> integrateingFn(double xi, double eta);

  std::pair<MatrixXd, MatrixXd> bMatrix(double xi, double eta);

public:
  FemPlateMitc9My(unsigned id, Node **nodes,
                  std::shared_ptr<AbstractElement> generalElement);

  FemPlateMitc9My(unsigned id, Node **nodes, const Material &material,
                  std::shared_ptr<AbstractElement> generalElement);
